    ...
    return (
        <section className={cn("pb-4 sm:pb-8", className)}>
            <div className="container mx-auto px-4 md:px-6">
                ...
                >
                    <ul 
                        className="flex items-center justify-center md:justify-start [&_li]:mx-2 sm:[&_li]:mx-3.5 
                        animate-infinite-scroll group-hover:[animation-play-state:paused]"
                    >
                        {partnerLogos.map((logo, index) => (
                            <li key={index} className="flex-shrink-0">
                                <div className="h-[72px] sm:h-24 w-[136px] sm:w-44 rounded-2xl border border-stone-200/80 bg-white flex items-center justify-center p-2.5 sm:p-4 shadow-2xs hover:border-stone-300 hover:shadow-xs transition-all duration-200 overflow-hidden">
                                    <Image 
                                        src={logo.src} 
                                        alt={logo.alt} 
                                        width={120} 
                                        height={50} 
                                        className="max-h-9 sm:max-h-12 max-w-[95px] sm:max-w-[118px] w-auto h-auto object-contain" 
                                    />
                                </div>
                            </li>
                        ))}
